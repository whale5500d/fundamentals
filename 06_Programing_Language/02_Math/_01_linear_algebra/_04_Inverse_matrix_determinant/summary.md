...
- 실행 결과
  ...
  ```bash
  역행렬·행렬식 속도 비교 (n×n 정방 행렬, 반복 횟수 = 3)

  --- n×n = 5×5 ---
  연산                  순수 Python (ms)     NumPy (ms)    speedup
  -------------------------------------------------------------
  inverse                     0.0391         0.0088       4.4x
  determinant                 0.0188         0.0160       1.2x

  --- n×n = 10×10 ---
  연산                  순수 Python (ms)     NumPy (ms)    speedup
  -------------------------------------------------------------
  inverse                     0.2219         0.0084      26.4x
  determinant                 0.0582         0.0044      13.2x

  --- n×n = 50×50 ---
  연산                  순수 Python (ms)     NumPy (ms)    speedup
  -------------------------------------------------------------
  inverse                    24.3087         0.0533     455.8x
  determinant                 3.8551         0.0248     155.5x

  --- n×n = 100×100 ---
  연산                  순수 Python (ms)     NumPy (ms)    speedup
  -------------------------------------------------------------
  inverse                   192.4908         0.1556    1237.2x
  determinant                28.0322         0.0567     494.3x
  /Users/imgyeonglak/Documents/01_career/01_dev_project/00_BASIC/06_math/.venv/lib/python3.14/site-packages/numpy/linalg/_linalg.py:2406: RuntimeWarning: overflow encountered in det
    r = _umath_linalg.det(a, signature=signature)

  --- n×n = 200×200 ---
  연산                  순수 Python (ms)     NumPy (ms)    speedup
  -------------------------------------------------------------
  inverse                  1547.4353         0.3662    4225.2x
  determinant               215.3480         0.7001     307.6x
  ```

세 가지 특징을 확인할 수 있음.

- **inverse가 determinant보다 일관되게 느림**: inverse는 [A|I] 구성으로 열이 2n개인
  확장 행렬을 처리하므로 루프 횟수가 determinant의 약 2배임. NumPy에서도 동일한 경향이
  나타나며, 내부적으로 `dgetrf` (LU 분해) 이후 inverse는 `dgetri`를 추가 호출하기 때문임.

- **n=200에서 inverse speedup이 4225x**: O(n³) 복잡도에서 3단계 선형 시스템(`solve()`)의
  n=200 speedup 307x보다 현저히 높음. inverse는 [A|I]의 오른쪽 절반(n열)에도 동일한
  행 연산을 적용하므로 Python 루프가 더 많이 발생해 순수 Python 구현의 절대 비용이
  더 크기 때문임.

- **RuntimeWarning — overflow in det (n=200)**: NumPy의 `det` 경고는 n=200 대각 우세
  행렬의 행렬식이 `float64` (배정밀도 부동소수점 — IEEE 754 표준, 64비트) 표현 범위
  (~1.8×10^308)를 초과할 만큼 매우 크기 때문에 발생함. 실용 코드에서는
  `numpy.linalg.slogdet` (행렬식의 로그(log) 값과 부호를 반환)를 사용해
  overflow (오버플로)를 회피함.

## 1.5 한계: 역행렬 직접 계산의 비효율성

- 역행렬을 구한 뒤 선형 시스템을 푸는 방식은 수치적으로 비효율적입니다. 역행렬 계산은 O(n³)이고 이후 행렬-벡터곱도 O(n²)인 반면, 3단계의 `solve()`는 O(n³) 한 번으로 동일한 결과를 얻습니다. 역행렬은 b가 자주 바뀌는 반복 풀이 맥락에서 사전 계산
  (precomputation)할 때만 효율적입니다.
