...
- 고유값과 고유벡터는 행렬이 표현하는 선형 변환(linear transformation)의 본질적인 방향과 크기를 드러내는 도구임. 선형 변환 A를 임의의 벡터 v에 적용하면 일반적으로 방향과 크기가 모두 바뀜. 그런데 특정 벡터들은 변환 후에도 방향이 유지되고 크기만 스칼라 배로 변함. 이 벡터가 고유벡터(eigenvector)이고, 그 스칼라 배율을 고유값(eigenvalue)이라 함.

- AI에서 등장하는 주요 맥락은 두 가지임.
  - PCA(Principal Component Analysis, 주성분 분석): 데이터의 공분산 행렬(covariance matrix)을 고유값 분해(eigendecomposition)하면, 고유벡터는 데이터가 가장 많이 퍼진 방향(principal component)을 나타내고, 고유값은 그 방향의 분산(variance) 크기를 나타냄. 가장 큰 고유값에 대응하는 고유벡터 방향으로 데이터를 투영(projection)하면 정보 손실을 최소화하여 차원을 축소할 수 있음.
...
- 정방 행렬(square matrix) A ∈ ℝ^(n×n)에 대해 다음 조건을 만족하는 스칼라 λ ∈ ℝ와 영벡터가 아닌 벡터 v ∈ ℝⁿ의 쌍을 고유쌍(eigenpair)이라 함.
...
- 이 식의 의미: 행렬 A가 벡터 v에 작용했을 때 방향은 유지되고 크기만 λ배가 됨.
...
v ≠ 0인 해가 존재하려면 (A - λI)가 singular(특이 행렬)이어야 함.
...
고유값 방정식을 직접 풀면 O(n³) 이상의 비용이 들고 구현이 복잡함. 거듭제곱법은 절댓값이 가장 큰 고유값(지배 고유값, dominant eigenvalue)과 대응하는 고유벡터를 반복적으로 근사하는 알고리즘임.
...
- 수렴 속도는 |λ₁/λ₂|에 의존함. λ₁이 지배 고유값, λ₂가 두 번째로 큰 고유값일 때 이 비율이 클수록 빠르게 수렴함.
